...
const RecordScreen = () => {
    ...
    const {startRecording,stopRecording,resetRecording,isRecording,recordedBlob,recordedVideoUrl,recordingDuration} = useScreenRecording()
    const closeModal = ()=>{
        resetRecording();
        setIsOpen(false);
    }
                            ...
                            <div className='record-box'>
                                {!isRecording && !recordedVideoUrl && (
                                    <button onClick={handleStart} className='record-start'>
                                        <Image src={ICONS.record} alt="record" width={16} height={16} />Record
                                    </button>
                                )}
                                {
                                    isRecording && (
                                        <button onClick={stopRecording} className='record-stop'>
                                            <Image src={ICONS.record} alt="stop" width={16} height={16} />Stop Recording
                                        </button>
                                    )
                                }

                                {
                                    recordedVideoUrl && (
                                        ...
                                        <button onClick={goToUpload} className='record-upload'>
                                            <Image src={ICONS.upload} alt="upload" width={16} height={16} />Continue to Upload
                                        </button>
                                        </>
                                    )
                                }
